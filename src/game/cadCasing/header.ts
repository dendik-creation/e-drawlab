...
/**
 * Title, "Langkah N" badge, home/back icons and the bgm toggle for the CAD
 * Casing PCB journey — same chrome shape as `JalurPcbHeader`/`DesainSkemaHeader`,
 * kept as its own class since none of the three journeys share an owner and the
 * only differences (title text, icon anchors) aren't worth threading a config
 * object through. Sliced from the Figma frame "Step 1 - Materi" (node 135:30).
 */
export class CadCasingHeader {
  ...
  /** `showBack` is false on Langkah 1 — the Figma frame's own "go_back" prop is hidden there too. */
  render(badgeLabel: string, showBack: boolean, entrance: boolean) {
    ...
    const title = scene.add
      .text(DESIGN_WIDTH / 2, TITLE_Y, 'Casing PCB yang Ideal', {
